...
function iw_main_user_main()
{
    ...
    $blockFlaggedDetails = pnModApiFunc('iw_main', 'user', 'userVarExists',
    ...
    $sv = pnModFunc('iw_main', 'user', 'genSecurityValue');
    $subscribeNews = pnModApiFunc('iw_main', 'user', 'userVarExists',
    ...
    //get the last cron successfull response
    $sv = pnModFunc('iw_main', 'user', 'genSecurityValue');
    $userMail = pnModFunc('iw_main', 'user', 'getUserInfo',
                           array('uid' => $uid,
                                 'sv' => $sv,
                                 'info' => 'e'));
    $sv = pnModFunc('iw_main', 'user', 'genSecurityValue');
    ...
    $photo_s = pnModFunc('iw_main', 'user', 'getUserPicture',
                          array('uname' => '_' . pnUserGetVar('uname'),
                                'sv' => $sv));
...
function iw_main_user_updateconfig($args)
{
    ...
    if ($details != null) {
        $sv = pnModFunc('iw_main', 'user', 'genSecurityValue');
        $result = pnModFunc('iw_main', 'user', 'userSetVar',
                             array('uid' => $uid,
    ...
    if ($deleteAvatar != 1) {
        ...
        pnModFunc('iw_main', 'user', 'deleteAvatar',
                   array('avatarName' => pnUserGetVar('uname'),
                         'extensions' => array('jpg',
                                               'png',
                                               'gif')));
        pnModFunc('iw_main', 'user', 'deleteAvatar',
                   array('avatarName' => pnUserGetVar('uname') . '_s',
                         'extensions' => array('jpg',
                                               'png',
                                               'gif')));
    }
...
function iw_main_user_news($args) {
    ...
    if (pnModFunc('iw_main', 'user', 'checkSecurityValue',
                   array('sv' => $sv))) {
        $requestByCron = true;
    }
    if ($uid == null) {
    ...
    if ($where != '') {
        $sv = pnModFunc('iw_main', 'user', 'genSecurityValue');
        $news = pnModFunc('iw_main', 'user', 'userGetVar',
                           array('uid' => $uid,
                                 'module' => 'iw_main_block_news',
                                 'name' => 'news',
                                 'sv' => $sv,
        ...
        $pos_end = pnModFunc('iw_main', 'user', 'stringrpos',
    ...
    if ($modinfo['state'] == 3 && ($where == 'ta' || $where == '')) {
        ...
        if (SecurityUtil::checkPermission('iw_noteboard::', "::", ACCESS_READ) || $requestByCron) {
            ...
            $noves = pnModAPIFunc('iw_noteboard','user','noves',
                                   array('uid' => $uid,
                                         'sv' => $sv));
            if ($noves['nombre'] > 0) {
                $out .= '<!---ta--->';
    ...
    if ($modinfo['state'] == 3 && ($where == 'ag' || $where == '')) {
        ...
        if (SecurityUtil::checkPermission('iw_agendas::', "::", ACCESS_READ) || $requestByCron) {
            ...
            $noves = pnModAPIFunc('iw_agendas','user','new',
                                   array('uid' => $uid,
                                         'sv' => $sv));
    ...
    if ($modinfo['state'] == 3 && ($where == 'me' || $where == '')) {
        if (SecurityUtil::checkPermission('iw_messages::', "::", ACCESS_READ) || $requestByCron) {
            ...
            $noves = pnModAPIFunc('iw_messages', 'user', 'countitems',
                                   array('uid' => $uid,
                                         'unread' => true,
                                         'sv' =>$sv));
            if ($noves > 0) {
    ...
    if ($modinfo['state'] == 3 && ($where == 'fo' || $where == '')) {
        if (SecurityUtil::checkPermission('iw_forums::', "::", ACCESS_READ) || $requestByCron) {
            ...
            $registres = pnModAPIFunc('iw_forums', 'user', 'getall',
                                       array('uid' => $uid,
                                             'sv' => $sv));
            $out .= '<!---fo--->';
            foreach ($registres as $registre) {
                ...
                if ($registre['actiu'] == 1 && pnModFunc('iw_forums', 'user', 'access',
                                                          array('fid' => $registre['fid'],
                                                                'uid' => $uid,    'sv' => $sv)) > 0) {
                    if ($uid != $realUid) {
                        $sv = pnModFunc('iw_main', 'user', 'genSecurityValue');
                    }
                    $noves = pnModAPIFunc('iw_forums', 'user', 'compta_msg',
                                           array('fid' => $registre['fid'],
                                                 'tots' => true,
                                                 'uid' => $uid,
                                                 'sv' => $sv));
    ...
    if ($modinfo['state'] == 3 && ($where == 'fu' || $where == '')) {
        ...
        foreach ($forms as $form) {
            ...
            if ($access['level'] > 1) {
                ...
                $nNotes = pnModAPIFunc('iw_forms', 'user', 'getNewNotes',
                                        array('fid' => $form['fid'],
                                              'uid' => $uid,
                                              'sv' => $sv));
                if (count($nNotes) > 0) {
    ...
    if (strpos($out,'<tr>') == '0') {$out = __('Nothing to show', $dom);}
...
function iw_main_user_flagged($args) {
    ...
    if ($where != '') {
        $sv = pnModFunc('iw_main', 'user', 'genSecurityValue');
        $flags = pnModFunc('iw_main', 'user', 'userGetVar',
                            array('uid' => pnUserGetVar('uid'),
                                  'module' => 'iw_main_block_flagged',
                                  'name' => 'flagged',
                                  'sv' => $sv,
    ...
    if ($modinfo['state'] == 3 && ($where == 'fo' || $where == '')) {
        if (SecurityUtil::checkPermission('iw_forums::', "::", ACCESS_READ)) {
            ...
            foreach ($forums as $forum) {
                $n_msg = pnModAPIFunc('iw_forums', 'user', 'compta_msg',
                                       array('fid' => $forum['fid'],
                                             'tots' => true));
            ...
            foreach ($forumsArray as $forum) {
                if ($forum['marcats'] > 0) {
                    $flagged = pnModAPIFunc('iw_forums','user','getFlagged',
                                             array('fid' => $forum['fid']));
                    $flaggedArray = array();
                    foreach ($flagged as $registre) {
                        if (pnModFunc('iw_forums', 'user', 'access',
                                       array('fid' => $registre['fid']))) {
        ...
        if (SecurityUtil::checkPermission('iw_forms::', "::", ACCESS_READ)) {
            ...
            foreach ($flagged as $flag) {
                    ...
                    $access = pnModFunc('iw_forms', 'user', 'access',
                                         array('fid' => $flag['fid']));
                    if ($access['level'] == 7) {
                            $formx = pnModAPIFunc('iw_forms', 'user', 'getFormDefinition',
                                                   array('fid' => $flag['fid']));
            ...
            foreach ($formsArray as $form) {
                    $flaggeds = pnModAPIFunc('iw_forms', 'user', 'getFlagged',
                                              array('fid' => $form['fid']));
                    $out .= '<tr>';
                    $out .= '<td align="left" valign="top">';
...
function iw_main_user_getAllUsersInfo($args)
{
    $dom = ZLanguage::getModuleDomain('iw_main');
    extract($args);
    $users = pnModApiFunc('iw_main', 'user', 'getAllUsers',
                           array('sv' => $sv,
                                 'fromArray' => $fromArray,
                                 'list' => $list));
    ...
    if ($modinfo['state'] == 3) {
        ...
        $sv = pnModFunc('iw_main', 'user', 'genSecurityValue');
        $extraUsers = pnModApiFunc('iw_main', 'user', 'getUsersExtraInfo',
                                    array('sv' => $sv,
                                          'fromArray' => $fromArray,
                                          'list' => $list));
    }
...
{
    $user = pnModApiFunc('iw_main', 'user', 'getUser',
                          array('uid' => $args['uid'],
                                'sv' => $args['sv']));
    // Checks if module iw_users is installed. In this case get the users extra information
    $modid = pnModGetIDFromName('iw_users');
    ...
    if ($modinfo['state'] == 3) {
        // Get users extra information because it probably exists
        $sv = pnModFunc('iw_main', 'user', 'genSecurityValue');
        $userExtraInfo = pnModApiFunc('iw_main', 'user', 'getUserExtraInfo',
                                       array('uid' => $args['uid'],
                                             'sv' => $sv));
    } else {
...
{
    $groups = pnModApiFunc('iw_main', 'user', 'getAllGroups',
                            array('sv' => $args['sv']));
...
function iw_main_user_getMembersGroup($args)
{
    ...
    $members = pnModApiFunc('iw_main', 'user', 'getMembersGroup',
                             array('gid' => $gid,
                                   'sv' => $sv));
    if (onlyId != 1) {
...
function iw_main_user_isMember($args)
{
    ...
    $isMember = pnModApiFunc('iw_main', 'user', 'isMember',
                              array('uid' => $uid,
                                    'gid' => $gid,
                                    'sgid' => $sgid,
                                    'sv' => $sv));
...
{
    extract($args);
    $groups = pnModApiFunc('iw_main', 'user', 'getAllGroupsInfo',
                            array('sv' => $sv));
    foreach ($groups as $group) {
...
function iw_main_user_getAllUserGroups($args)
{
    ...
    $userGroups = pnModApiFunc('iw_main', 'user', 'getAllUserGroups',
                                array('sv' => $sv,
                                      'uid' => $uid));
    foreach ($userGroups as $group) {
        $items[$group['gid']] = array('id' => $group['gid']);
    ...
    if (!move_uploaded_file($file['tmp_name'], $filePath)) {
        ...
        if (($widthImg != 0 ||
            ...
            ($file_extension == 'gif' || $file_extension == 'jpg' || $file_extension == 'png')) {
                $sv = pnModFunc('iw_main', 'user', 'genSecurityValue');
                pnModFunc('iw_main', 'user', 'thumbnail',
                           array('sv' => $sv,
                                 'imgSource' => $filePath,
                                 'imgDest' => $filePath,
                                 'widthImg' => $widthImg,
...
function iw_main_user_downloadFile($args)
{
    ...
    if (!file_exists($folder . '/' . $fileNameInServer)) {
        // Create output object
        $pnRender = pnRender::getInstance('iw_main',false);
        $pnRender->assign('file', $folder . '/' . $fileName);
...
function iw_main_user_userGetVar ($args) {
    ...
    $value = pnModApiFunc('iw_main', 'user', 'userGetVar',
                           array('module' => $module,
                                 'name' => $name,
                                 'uid' => $uid,
                                 'sv' => $sv));
    ...
    if (count($value) > 0) {
        if (!$nult) {
            $sv = pnModFunc('iw_main', 'user', 'genSecurityValue');
            pnModApiFunc('iw_main', 'user', 'userUpdateGetVarTime',
                          array('sv' => $sv,
                                'uid' => $uid,
                                'module' => $module,
                                'name' => $name));
            ...
            $sv = pnModFunc('iw_main', 'user', 'genSecurityValue');
            pnModApiFunc('iw_main', 'user', 'userUpdateNultVar',
                          array('sv' => $sv,
                                'uid' => $uid,
                                'module' => $module,
                                'name' => $name));
    ...
    $sv = pnModFunc('iw_main', 'user', 'genSecurityValue');
    pnModApiFunc('iw_main', 'user', 'userDeleteOldVars',
                  array('sv' => $sv));
...
function iw_main_user_userSetVar ($args) {
    ...
    $valueExists = pnModApiFunc('iw_main', 'user', 'userVarExists',
                                 array('sv' => $sv,
                                       'uid' => $uid,
                                       'module' => $module,
                                       'name' => $name));
    ...
    if ($valueExists) {
        ...
        $result = pnModApiFunc('iw_main', 'user', 'updateUserVar',
                                array('sv' => $sv,
                                      'uid' => $uid,
                                      'module' => $module,
                                      'name' => $name,
        ...
        // The value is updated
        $result = pnModApiFunc('iw_main', 'user', 'createUserVar',
                                array('sv' => $sv,
                                      'uid' => $uid,
                                      'module' => $module,
                                      'name' => $name,
...
function iw_main_user_userInitVar ($args) {
    ...
    $sv = FormUtil::getPassedValue('sv', isset($args['sv']) ? $args['sv'] : null, 'POST');
    if ($lifetime != 0) {$lifetime = time() + 24*60*60*pnModGetVar('iw_main', 'usersvarslife');}
    $value = pnModFunc('iw_main', 'user', 'userGetVar',
                        array('sv' => $sv,
                              'module' => $module,
                              'name' => $name,
                              'uid' => $uid));
...
function iw_main_user_usersVarsDelModule ($args) {
    ...
    $result = pnModApiFunc('iw_main', 'user', 'usersVarsDelModule',
                            array('sv' => $sv,
                                  'module' => $module,
                                  'name' => $name));
    return $result;
...
function iw_main_user_userDelVar ($args) {
    ...
    $sv = FormUtil::getPassedValue('sv', isset($args['sv']) ? $args['sv'] : null, 'POST');
    //The variable is deleted
    $result = pnModApiFunc('iw_main', 'user', 'userDelVar',
                            array('sv' => $sv,
                                  'module' => $module,
                                  'uid' => $uid,
                                  'name' => $name));
...
function iw_main_user_regenBlockNews ($args) {
    ...
    $sv = pnModFunc('iw_main', 'user', 'genSecurityValue');
    //The variable is deleted
    $result = pnModApiFunc('iw_main', 'user', 'regenDinamicVars',
                            array('sv' => $sv,
                                  'uid' => $uid));
